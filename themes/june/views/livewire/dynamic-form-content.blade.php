<form wire:submit.prevent="submit" method="POST" novalidate>
    ...
    <div class="row g-2">
        @foreach ($formFields as $fieldName => $fieldConfig)
            @php
                $colClass =
                    !empty($displayOptions['columns']) && $displayOptions['columns'] == 2 ? 'col-md-6' : 'col-12';
                    ...
                    @elseif($fieldConfig['type'] === \App\Enums\FormFieldType::Textarea)
                        <div class="form-group">
                            ...
                            <textarea id="{{ $fieldName }}" class="form-control @error($errorName) is-invalid @enderror"
                                ...
                                placeholder="{{ $fieldConfig['placeholder'] ?? '' }}" {{ $fieldConfig['required'] ?? false ? 'required' : '' }}></textarea>
                            @error($errorName)
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
    ...
    <div class="mt-4 text-center">
        <button type="submit" class="btn btn-dark px-4 py-2" wire:loading.attr="disabled">
            ...
            <span wire:target="submit" class="d-inline-flex align-items-center d-none"
